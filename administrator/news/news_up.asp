...
'	Upload.SetMaxSize 100000000, True 'Dat kich co upload la` 1MB
	...
	if iStatus	=	"edit" then
        ...
        if languageid = "" then languageid = "VN"
   ...
   End if
    if iStatus	=	"add" then
				...
				sql="insert into News (NewsID, idcode,url_video, Title, Description,Author, DecsBannerImage, Body,IsHomeNews, IsCatHomeNews"
                sql = sql & " ,IsCatHomeNews_Below, IsHotNews, AdsHome, AdsNews, PictureID, Source , Price, Unit, StoreOf, Size, EmptyStore, Note,meta_keyword,meta_desc, Creator, CreationDate, LastEditor, LastEditedDate, Status,languageid)"
				sql = sql & " values ("
				sql=sql & NewsId
				sql=sql & ",N'" & idcode & "'"
                sql=sql & ",N'" & url_video & "'"
