...
'	Upload.SetMaxSize 100000000, True 'Dat kich co upload la` 1MB
	...
	if iStatus	=	"edit" then
		...
		if Upload.Form("NewsId")<>"" and isnumeric(CLng(Upload.Form("NewsId"))) then
			NewsId = CLng(Upload.Form("NewsId"))
		else
			NewsId = 0
		end if
		'sCatId
		if Upload.Form("sCatId")<>"" and isnumeric(CLng(Upload.Form("sCatId"))) then
   ...
   ' response.write CatId
    ...
    if iStatus	=	"add" then
        ...
        NewsId=GetMaxId("News", "NewsId", "")         
                                                                       ...
                                                                       'IsSukien 'IsShare  
        sql="insert into News (NewsID, idcode, Title, Description,url_video,Author, DecsBannerImage, Body,IsHomeNews,IsHotNews, PictureID , Price,PriceNet, Unit, StoreOf, Size,Weight, EmptyStore,meta_keyword,meta_desc, CreationDate, LastEditedDate, Status,languageid)"
