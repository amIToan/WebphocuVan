...
<%
	...
	if request.Form("action")="Delete" then
		'Kiểm tra xem tin này có nằm ở nhiều chuyên mục không?
		sql="Select Count(NewsId) as dem from NewsDistribution where newsId=" & NewsId
		rs.open sql,con,1
		dem=Cint(rs("dem"))
		rs.close		
		if dem=1 then
		'Nếu dem=1, tức là tin chỉ nằm ở một chuyên mục thì xóa cả ở 2 bảng News và NewsDistribution
			sql="Delete NewsDistribution where NewsId=" & NewsId & ";Delete News where NewsId=" & NewsId
			rs.open sql,con,1
		else
		'Nếu dem>1, tức là tin chỉ nằm ở nhiều chuyên mục. 
		'Để không ảnh hưởng đến tin ở các chuyên mục khác. Chỉ xóa ở bảng NewsDistribution
			sql="Delete NewsDistribution where NewsId=" & NewsId & " and CategoryId=" & CatId
			rs.open sql,con,1
		end if
		set rs=nothing
		...
		Title=rs("Title")
		Categoryname=GetListParentCatNameOfCatId(CatId)
		rs.close
		set rs=nothing
	end if
%>
...
<table width="98%" border="0" cellspacing="2" cellpadding="2">
  <tr> 
    <td height="25" colspan="2" align="center" valign="middle"> <p><font size="2" face="Arial, Helvetica, sans-serif"><strong>Bạn 
        chắc chắn muốn xóa tin</strong></font></p></td>
  </tr>
  <tr> 
    <td colspan="2" align="center"><font size="2" face="Arial, Helvetica, sans-serif"><%=Title%></font></td>
  </tr>
  <tr> 
    <td colspan="2" align="center"><font size="2" face="Arial, Helvetica, sans-serif"><b>Chuyên mục:&nbsp;</b><%=Categoryname%></font></td>
  </tr>
  <tr> 
