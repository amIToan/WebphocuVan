...
function insertTable()
{
  ...
  var urlx= QBPATH + '/open/InsertTable.htm'
...
function insertTableIE()
{
  ...
  var arr=showModalDialog(urlx, '', "font-family:Verdana;font-size:12;dialogWidth:31em;dialogHeight:26em; edge:sunken;help:no;status:no");
  if(arr==null) return;
...
function tableProp()
{
  ...
  var urlx= QBPATH + '/open/TableProperties.htm'
...
function InsertImage()
{
	...
	if (isImageSelected())
	{
		window.open ('../news/news_chooseimage.asp?action=modify','','width=' + w + ',height=' + h + ',left=' + ((chasm - w - 10) * .5) + ',top=' + ((mount - h - 30) * .5) + ',directories=no,status=no,scrollbars=yes,resizable=yes, menubar=no');
	}
	else
	{
		window.open ('../news/news_chooseimage.asp?action=new','','width=' + w + ',height=' + h + ',left=' + ((chasm - w - 10) * .5) + ',top=' + ((mount - h - 30) * .5) + ',directories=no,status=no,scrollbars=yes,resizable=yes, menubar=no');
	}
}
function InsertAV()
{
	...
	var w = 750;
	var h = 500;
	window.open ('../news/news_chooseAV.asp?action=new','','width=' + w + ',height=' + h + ',left=' + ((chasm - w - 10) * .5) + ',top=' + ((mount - h - 30) * .5) + ',directories=no,status=no,scrollbars=yes,resizable=yes, menubar=no');
}
function InsertNewImage(imageURL)
{
  var el=document.frames[fID];
  el.focus();
  ...
  if(sel.type=="Control") 
  {
  	Range.item(0).outerHTML = imageURL; 
  }
  else
  {
  	Range.pasteHTML(imageURL);
  }
}//End function insert image
function InsertNewFile(FileURL)
{
  var el=document.frames[fID];
  el.focus();
  var sel = el.document.selection;
  ...
  if(sel.type=="Control") 
  {
  	Range.item(0).outerHTML = "<IMG " + FileURL + ">"; 
  }
  else
  {
  	Range.pasteHTML(FileURL);
  }
}//End function insert image
...
function InsertFiles()
{
	...
	var w = 600;
	var h = 500;
	window.open ('../news/file_upload.asp','','width=' + w + ',height=' + h + ',left=' + ((chasm - w - 10) * .5) + ',top=' + ((mount - h - 30) * .5));
}
function isImageSelected()
{
	var el=document.frames[fID];
	if (el.document.selection.type == "Control") {
		var oControlRange = el.document.selection.createRange();
		if (oControlRange(0).tagName.toUpperCase() == "IMG") {
			selectedImage = el.document.selection.createRange()(0);
			return true;
		}
	}
}