...
<%
	...
	Av_id=GetNumeric(Request.querystring("param"),0)
	Dim rs
	if Request.Querystring("action")="Insert" then
		...
		'Av_id
		Av_id=GetNumeric(Upload.form("Av_id"),0)
		'Av_Title
		...
		if not sError then
			...
			sql="Update AudioVideo Set" &_
				" Av_Title=N'" & Av_Title & "'" &_
				",Av_Path='" & Av_Path & "'"
				if Av_ImagesPath<>"" then
					sql=sql & ",Av_ImagesPath='" & Av_ImagesPath & "'"
				end if
			sql=sql & ",Av_Time=" & Av_Time &_
				",Av_Capacity=" & Av_Capacity &_
				",Av_LinkPopUp='" & Av_LinkPopUp & "'" &_
				",Av_Type=" & Av_Type &_
				",Av_Author=N'" & Av_Author & "'" &_
				",Av_Source=N'" & Av_Source & "'" &_
				",StatusId='" & StatusId & "'" &_
				",Av_Note=N'" & Av_Note & "'" &_
				",Av_LastEditor=N'" & session("User") & "'" &_
				",Av_LastEditedDate='" & now() & "'" &_
				" Where av_id=" & av_id
		...
		Set rs=server.CreateObject("ADODB.Recordset")
		sql="Select * from AudioVideo where av_id=" & av_id
		rs.open sql,con,1
			Av_Title=Trim(rs("Av_Title"))
			Av_Path=Trim(rs("Av_Path"))
			Av_ImagesPath=Trim(rs("Av_ImagesPath"))
			Av_Time=rs("Av_Time")
			Av_Capacity=Clng(rs("Av_Capacity"))
			Av_LinkPopUp=Trim(rs("Av_LinkPopUp"))
			Av_Type=Clng(rs("Av_Type"))
			Av_Author=Trim(rs("Av_Author"))
			Av_Source=Trim(rs("Av_Source"))
			StatusId=Trim(rs("StatusId"))
			Av_Note=Trim(rs("Av_Note"))
		rs.close
		set rs=nothing
  ...
  <table width="100%" border="0" cellspacing="2" cellpadding="1">
    <tr align="center" valign="middle"> 
      <td height="30" colspan="2">
      	<font size="3" face="Arial, Helvetica, sans-serif"><strong>Chỉnh sửa Tin Audio-Video</strong></font><br>
    ...
    <tr> 
      ...
      <td><input name="Av_Title" type="text" id="Av_Title" size="35" maxlength="500" value="<%=Av_Title%>">
    ...
    <tr> 
      ...
      <td><Select name="Av_Type">
      		<option value="1"<%if Av_Type=1 then%> selected<%end if%>>Window Media Video (*.wmv)</option>
      		<option value="0"<%if Av_Type=0 then%> selected<%end if%>>Window Media Audio (*.wma)</option>
      		<option value="2"<%if Av_Type=2 then%> selected<%end if%>>Real Media (*.rm)</option>
      		<option value="2"<%if Av_Type=3 then%> selected<%end if%>>AVI (*.avi)</option>
      		<option value="2"<%if Av_Type=4 then%> selected<%end if%>>Mp3(*.mp3)</option>
      	</Select>
      	<font size="2" face="Arial, Helvetica, sans-serif">(<strong><font color="#FF0000">*</font></strong>)</font>
      </td>
    </tr>
    ...
    <tr> 
      <td colspan="2" align="center"> 
		...
		<input type="Button" name="Upload" value="Upload" onClick="Javascript: window.open('Av_Uploadfile.asp?id=<%=Av_id%>&Av_Type=<%=Av_Type%>','_self');">
